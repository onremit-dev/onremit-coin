...
The OnRemit Core daemon can be configured to act as a trusted "border
router", implementing the onremit wire protocol and relay, making
...
The ZeroMQ feature in OnRemit Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building onremitd:
...
For instance:
    ...
    $ onremitd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/onremitd.tx.raw \
...
These options can also be provided in onremit.conf.
...
From the perspective of onremitd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into onremitd directly. Furthermore, no information is
...
using. OnRemitd appends an up-counting sequence number to each
