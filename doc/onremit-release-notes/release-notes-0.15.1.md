OnRemit Core version *0.15.1* is now available from:

  <https://download.onremit.org/onremit-0.15.1/>
...
A number of changes to the way OnRemit Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 notes per kB (0.00001 ONREMIT). This is to prepare for dropping the minimum transaction fee to 0.1 notes per kB (0.0001 ONREMIT) in the next release.
