OnRemit Core version 0.13.2 is now available from:

  <https://download.onremit.org/onremit-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a onremit
...
ECDSA signatures inside OnRemit transactions now use validation using
...
Previous versions of OnRemit Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of OnRemit Core was to raise the default minimum
relay fee.

OnRemit Core 0.13.2 will have a strict maximum size on the mempool. The
...
OnRemit Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running onremitd without having to do any manual configuration.
...
OnRemit Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. OnRemit Core will relay transactions with
...
entirely in OnRemit Core 0.13, and it is at this time undecided whether the
...
OnRemit Core automatically creates a hidden service to listen on, without
manual configuration. OnRemit Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if OnRemit Core is listening, and
...
OnRemitd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals OnRemit
Core to use floating fees. By default, OnRemit Core will use floating
fees.
...
(default: `0.0002` ONREMIT/kB).

At all times, OnRemit Core will cap fees at `-maxtxfee=<x>` (default:
0.10) ONREMIT.
Furthermore, OnRemit Core will never create transactions paying less than
...
`onremit.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /onremitrpc>
        ...
        # optional bypass onremitd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< onremitrpc:<password>
    </Location>
...
Restarting onremitd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in onremit.conf
...
For example:
    ...
    $ src/onremit-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The OnRemit Core code base has
...
- `onremit-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `onremit-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the OnRemit scripting system that in combination with
...
OnRemit transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of OnRemit Core.
...
The two phases are distinct in the OnRemit-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming OnRemit wallet
  software more difficult and which seriously complicates the design of smart
  contracts for OnRemit.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each OnRemit block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of onremits they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of onremits being spent is exactly
...
- **Increased security for multisig:** OnRemit addresses (both P2PKH addresses
  ...
  believed to be provided by OnRemit's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of OnRemit's current ledger (called the UTXO set),
  ...
  note that OnRemit Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  OnRemit users to individually opt-in to almost any change in the OnRemit
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and OnRemit Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the OnRemit
client to OnRemit Core 0.13.2 to signal for segwit support. When a
...
Since OnRemit Core 0.10.0, nodes have defaulted to only relaying and
...
- `onremit-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `onremit-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
