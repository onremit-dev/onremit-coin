    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional ONREMIT outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes ONREMIT and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::ONREMIT_TO_ONREMIT) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a ONREMIT-to-ONREMIT transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::ONREMIT_ONLY;
        ...
        // If ONREMIT-to-ONREMIT fails, create a peg-out transaction
